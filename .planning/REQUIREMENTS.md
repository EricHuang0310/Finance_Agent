...
- [ ] **TEAMS-02**: Each agent role has a `.claude/agents/*.md` definition with YAML frontmatter specifying model tier, tools, and permissions
- [ ] **TEAMS-03**: Tiered model assignment enforced per role: Opus for CIO/Judge/Reflection, Sonnet for analysts/debate, Haiku for data collection/execution
...
- [ ] **MEM-01**: Fix silent memory corruption (replace pass-through exception handling with proper error reporting)
...
- [ ] **MEM-04**: Fix trade log race condition for concurrent Agent Teams execution
